...
Column 1 is the group ID, column 2 symbolized if the data is before or after the meetings, column 3 is the condition in words (sick or healthy), column 4 is the numeric of participnats in each condition, column 5 is the ratio of the infected/unifected within each group.
...
infectionModel <- function(Groups = 4, Members = 4, initialProbToIll = 0.5, numberOfMeetings = 2){
  ...
  adjPToIll <- function(iPTI = initialProbToIll, N = data3){
    Sick <- 1
    L <- length(N == Sick)
    S <- 0
    for (i in 1:length(L)){
